...
DDPClient.prototype._message = function(data) {
  ...
  var cb = self._subscriptionCallbacks[data.collection];
  if(cb) {
    cb(self.collections[data.collection], data);
  };
};
...
DDPClient.prototype.unsubscribe = function(id) {
  ...
  delete self._subscriptionCallbacks[id];

  self._send({msg: 'unsub', id: id});
};

DDPClient.prototype.addSubscriptionCallback = function(collection, callback) {
  this._subscriptionCallbacks[collection] = callback;
};

DDPClient.prototype.removeSubscriptionCallback = function(collection, callback) {
  delete this._subscriptionCallbacks[collection];
};
